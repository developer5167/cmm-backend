...
const sendOTPHandler = async (req, res, next) => {
  try {
    ...
    // Generate & store OTP (Using static OTP for testing)
    ...
    // Send OTP (Paused for testing static OTP)
  ...
  try {
    ...
    const userResult = await query(
      `INSERT INTO users (phone_number, country_code, is_phone_verified)
       ...
       (SELECT review_status FROM user_profiles WHERE user_id = users.id) as review_status`,
    ...
    res.json({
      ...
      data: {
        ...
        user: {
          ...
          review_status: user.review_status || 'pending',
        },
...
const adminLoginHandler = async (req, res, next) => {
  try {
    ...
    const result = await query(
      'SELECT id, password_hash, is_active, role FROM admin_users WHERE email = $1',
      [email]
    );
    ...
    const isMatch = await require('bcryptjs').compare(password, admin.password_hash);
    if (!isMatch) {
      return res.status(401).json({ success: false, message: 'Invalid credentials' });
    }

    // Generate tokens
    const accessToken = generateAccessTokenAdmin(admin.id, admin.role);
    const refreshToken = generateRefreshTokenAdmin(admin.id);
