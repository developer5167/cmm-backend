...
const step3 = async (req, res, next) => {
  try {
    const { education, profession, annual_income_min, annual_income_max, height_cm, complexion, native_place, languages_spoken, previously_married, marriage_timeline } = req.body;
    const userId = req.user.id;
...
const step5 = async (req, res, next) => {
  try {
    const { father_occupation, mother_occupation, brothers_count, sisters_count, married_brothers_count, married_sisters_count, family_income_range, family_class } = req.body;
    const userId = req.user.id;
...
const step6 = async (req, res, next) => {
  try {
    const { age_min, age_max, location_flexible, preferred_locations, denomination_flexible, preferred_denominations, caste_flexible, preferred_castes, education_preference, profession_preference, salary_min, salary_max } = req.body;
    const userId = req.user.id;

    const fields = Object.fromEntries(
      Object.entries({
        age_min, age_max, location_flexible, preferred_locations,
...
const step10 = async (req, res, next) => {
  try {
    ...
    const bio = await generateBio(profileRes.rows[0] || {});
    await upsertProfile(userId, { bio });
